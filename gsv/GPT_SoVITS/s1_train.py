...
# torch>=2.6 默认 weights_only=True，旧 ckpt 内含 pathlib 对象，不加白名单无法加载/断点续跑
import pathlib as _pathlib
try:
    torch.serialization.add_safe_globals([
        _pathlib.WindowsPath, _pathlib.PosixPath, _pathlib.Path,
        _pathlib.PurePath, _pathlib.PureWindowsPath, _pathlib.PurePosixPath,
    ])
except Exception:
    pass
...
def main(args):
    ...
    trainer: Trainer = Trainer(
        ...
        benchmark=False,
        fast_dev_run=False,
        # 单卡不要用 DDP/gloo（torch>=2.9 在 Windows 单卡 DDP 会原生崩溃 0xC0000005）
